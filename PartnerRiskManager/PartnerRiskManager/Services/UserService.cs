...
namespace PartnerRiskManager.Services
{
    public class UserService : IUserService
    {
        ...
        public async Task<IEnumerable<ApplicationUser>> GetAllUsersWithoutPartnerAsync()
        {
            return await _db.ApplicationUsers.Where(u => u.Partner == null).ToListAsync();
        }
        ...
        {
            var allUsers = await _db.Users.Include(u => u.Partner).ToListAsync();
        ...
        public async Task<List<ApplicationUser>> SearchUsersWithNoPartner(string str)
        {
            var allUsers = await _db.Users.Where(u => u.Partner == null).ToListAsync();
            var usersToReturn = new List<ApplicationUser>();
            var searchTerms = str.ToLower().Split(' ');
        ...
        public async Task<PaginationDto<ApplicationUser>> GetUsersByPageAsync(int page, int pageSize)
        {
            ...
            return new PaginationDto<ApplicationUser>
            {
                Items = await query.Skip((page - 1) * pageSize).Take(pageSize).Include(u => u.Partner).ToListAsync(),
                TotalPages = (int)Math.Ceiling(count / (double)pageSize),
                CurrentPage = page
            };
        }
