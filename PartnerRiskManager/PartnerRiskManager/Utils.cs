...
namespace PartnerRiskManager
{
    public static class Utils
    {
        ...
        public static List<Partner> PartnerStreamToList(MemoryStream stream)
        {
            ExcelPackage.LicenseContext = LicenseContext.NonCommercial;
            using (var package = new ExcelPackage(stream))
            {
                return ParsePartnersExcel(package);
            }
        }

        public static List<Partner> ParsePartnersExcel(ExcelPackage package)
        {
        ...
        public static ExcelPackage ExportPartnersExcel(IEnumerable<Partner> partners)
        {
            ExcelPackage.LicenseContext = LicenseContext.NonCommercial;
            ExcelPackage excel = new ExcelPackage();
            var workSheet = excel.Workbook.Worksheets.Add("Partners");
            ...
            {
                workSheet.Cells[recordIndex, 1].Value = partner.Brand;
                workSheet.Cells[recordIndex, 2].Value = partner.Model;
                workSheet.Cells[recordIndex, 3].Value = partner.ChassisSeries;
                workSheet.Cells[recordIndex, 4].Value = partner.LicencePlate;
                workSheet.Cells[recordIndex, 5].Value = partner.FirstRegistrationDate.ToString();
                workSheet.Cells[recordIndex, 6].Value = partner.Color;
                workSheet.Cells[recordIndex, 7].Value = partner.Mileage;
